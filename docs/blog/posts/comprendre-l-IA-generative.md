...
L'arrivée de ChatGPT a vraiment changé la donne dans le monde de l'IA. Contrairement à ce qu'on pourrait croire, la recette (une fois revelée) n'est pas si compliquée : il s'agit surtout d'utiliser une énorme quantité de données et beaucoup de puissance de calcul pour entraîner des modèles très grands, qu'on appelle des "Transformers". Ces modèles ne sont pas nouveaux, ils existent depuis [2017](https://arxiv.org/abs/1706.03762). Ce qui a tout bouleversé, c'est la taille à laquelle ils ont été utilisés. 
